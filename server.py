...
def clientThread(chatUser):
    ...
    while True:
        ...
        if not data:
            for x in connlist:
                if x.adres <> chatUser.adres: x.connection.send("User " + chatUser.username.rstrip() + " disconnected")
        ...
        for x in connlist:
            if x.adres <> chatUser.adres: x.connection.send("[" + chatUser.adres[0] + "] " + chatUser.username.rstrip() + ": " + data.rstrip())
